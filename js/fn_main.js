...
var res2 = [
    {
       ...
       for(var i=0;i<res.length;i++){
           ...
           ol
           ol.append(
               $('<li/>').append($('<img/>').prop('src','./imgs/img/'+ i + '.gif')).append($('<span/>').html(obj.title))
           ).append(
               $('<li/>')
                   .append($('<a/>').html(obj.mainCity[0]))
                   .append($('<a/>').html(obj.mainCity[1]))
                   .append($('<a/>').html(obj.mainCity[2]))
                   .append($('<a/>').html(obj.mainCity[3]))
                   .append($('<a/>').html(obj.mainCity[4]))
           );
           ol.data("moreCity",obj.moreCity).mouseover(function(event){
               var pd = $(this).data("moreCity");
               $(".content").empty();
               $('.content').css('width', '700px');
               var heightR = 0;
               var div_1 = $('<div/>');
               var div_2 = $('<div/>');
               $(".content").append(div_1).append(div_2);
                //遍历moreCity，将里边内容动态添加进div1或div2
                for(var j=0;j<pd.length;j++){
                    var mc = pd[j];
                    heightR += div_1.height();
                    //进行添加到div1 与div2 中
                    if(heightR>352){
                        var h2 = $("<h3/>").html(mc.cityName).appendTo(div_2);
                        //为City下的items创建a
                        for(var l = 0;l<mc.items.length;l++){
                            var a = $('<a/>').html(mc.items[l]).appendTo(div_2)
                        }
                    }else{
                        var h2 = $("<h3/>").html(mc.cityName).appendTo(div_1);
                        for(var l = 0;l<mc.items.length;l++){
                            var a = $('<a/>').html(mc.items[l]).appendTo(div_1);
                        }
                    }
                }
               //将图片添加进第二个div
               $('<img/>').prop('src',res[$(this).index()].moreCityImg).appendTo(div_2)
                   .css('width','90%').css('height','90%').css('margin-top','20px').css('float','right')
               //如果菜单右侧只有两个栏，设置样式
               if(pd.length == 2){
                   $(".content div").empty();
                   var h2 = $("<h3/>").html(pd[0].cityName).appendTo(div_1);
                   for(var l = 0;l<pd[0].items.length;l++){
                       var a = $('<a/>').html(pd[0].items[l]).appendTo(div_1)
                   }
                   var h3 = $("<h3/>").html(pd[1].cityName).appendTo(div_2);
                   for(var l = 0;l<pd[1].items.length;l++){
                       var b = $('<a/>').html(pd[1].items[l]).appendTo(div_2)
                   }
                   $('<img/>').prop('src',res[3].moreCityImg).appendTo(div_2)
                       .css('width','90%').css('height','90%').css('margin-top','20px').css('float','right')

               }
               //    为第六个添加特殊格式
               var a = this.childNodes[0].innerText;
               if(a == '主题推荐'){
                   menu_R('主题推荐')
               }
           });
           $('.menu ul').append(ol);
           //设置当鼠标在左侧菜单上时，右边的页面显示出来
           $('ol').mouseenter(function(){
               $(this).append( $('.content').css({
                   top: 10,
                   left:240
               }).show());
...
// })
//第六个特殊格式显示
function menu_R(Title) {// '<span>主题推荐</span>'
    if (Title == '主题推荐') {
        $(".content").empty();
        $('.content').css('width', '400px');
        var h2 = $("<h1/>").html(Title).appendTo($('.content'));
        $('<img/>').prop('src', "http://ww1.sinaimg.cn/large/7853084cjw1f86txt0owej204g04gweg.jpg")
            .appendTo($('.content'));
        $('<img/>').prop('src', 'http://ww2.sinaimg.cn/large/7853084cjw1f86txycahcj204g04gq2v.jpg')
            .appendTo($('.content'));
        $('<img/>').prop('src', 'http://ww1.sinaimg.cn/large/7853084cjw1f86tybhx5zj204g04gdfv.jpg')
            .appendTo($('.content'));
        $('<img/>').prop('src', 'http://ww1.sinaimg.cn/large/7853084cjw1f86txt0owej204g04gweg.jpg')
            .appendTo($('.content'));
        $('<img/>').prop('src', 'http://ww1.sinaimg.cn/large/7853084cjw1f86tyhut5pj204g04gq2w.jpg')
            .appendTo($('.content'));
        $('<img/>').prop('src', 'http://ww2.sinaimg.cn/large/7853084cjw1f86tyoduc3j204g04g3yi.jpg')
            .appendTo($('.content'));
        $('<img/>').prop('src', 'http://ww3.sinaimg.cn/large/7853084cjw1f86tyxm9w8j204g04gjrd.jpg')
            .appendTo($('.content'));
        $('<img/>').prop('src', 'http://ww1.sinaimg.cn/large/7853084cjw1f86tz4g8qdj204g04gwef.jpg')
            .appendTo($('.content'));
        $('<img/>').prop('src', 'http://ww3.sinaimg.cn/large/7853084cjw1f86tza3mvqj204g04gmx5.jpg')
            .appendTo($('.content'));
        $('.content img').css('width','100px').css('height','100px').css('margin-left','30px').css("margin-bottom",'30px')
    }
}
//为ol添加进图片????????????????????
function addLogo(  ){
    ...
    //console.log(count)
    switch (i){
        case '0' :$('<img/>').prop('src','public/imgs/img/ul.gif').appendTo($('ol li:eq(0)').child(span).before());console.log(1);break;
        case 1 :$('<img/>').prop('src','public/imgs/img/ul——_03.gif');break;
        case 2 :$('<img/>').prop('src','public/imgs/img/ul——_06.gif');break;
        case 3 :$('<img/>').prop('src','public/imgs/img/ul——_08.gif');break;
        case 4 :$('<img/>').prop('src','public/imgs/img/ul——_10.gif');break;
        case 5 :$('<img/>').prop('src','public/imgs/img/ul——_12.gif');break;
        case 6 :$('<img/>').prop('src','public/imgs/img/ul2.gif');break;
    }
    count++
}
...
// $.ajax({
    ...
    // success : function(res){
        ...
        var p2 = $('<ul/>').appendTo($('#main .area_1')).prop('class','z-tab').prop('id','tab1');
        var show_ = $('<div/>').prop('class','show').appendTo($('#main .area_1'))
        for(var i=0;i<res2.length;i++){
            var obj = res2[i];
            var a = $("<li/>").html(res2[i].title).appendTo(p2)
        //    绑定a与data的事件
            a.data('data',obj.data).mouseover(function(event){
                var pd = $(this).data('data')
                show_.empty();
                for(var j=0;j<pd.length;j++){
                    var mc = pd[j];
                    var dl = $("<dl/>").append(
                        $("<dt/>").append($("<img/>").prop('src',mc.imgUrl).css('width','100%').css('height','40%'))
                    ).append(
                        $('<dd/>').html(mc.price)
                    ).append(
                        $("<dd/>").html(mc.title)
                ...
                //    为第一个ul创建特殊格式
                $('.show dl:eq(0) img').css('height','270px')
                //添加最后一个ul
                ul_last()
        ...
        function ul_last(){
            ...
            ul_.append(
                ...
                $("<li/>").append($('<a/>').html('机票')).append($('<a/>').html('酒店')).append($('<a/>').html('机+酒')).append($('<a/>').html('游轮'))
            )
            $('.show').append(ul_)
        }
    // }
...
//cityWalkList.json --- ajax
$('#nav span:eq(9)').click(function(){
    $.ajax({
        url : '/cityWalk',
        success : function (res){
            window.open('cityWalk','_self')
        }
    })
})
